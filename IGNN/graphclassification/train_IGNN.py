...
path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', args.dataset)
...
'''
For real-world dataset, need to load data manually in order to get the degrees, edges, subgraph labels etc.
Instead of using TUDataset as this line will throw an error if run with arg --dataset "NOT MUTAG"
'''
dataset = TUDataset(root = path, name=args.dataset)
if len(dataset) == 0:
    raise ValueError("The dataset is empty. Please check the dataset files in the specified directory.")
dataset.shuffle()
...
for fold_idx in range(10):
    ...
    model = IGNN(nfeat=dataset.num_features,
                nhid=args.hidden,
