...
class IGNN(nn.Module):
    ...
    '''
    def forward(self, features,adj, subgraph_masks):
        x = features

        # ImplicitGraph layers
        x = self.ig1(self.X_0, adj, x, F.relu, self.adj_rho, A_orig=self.adj_orig)
        x = self.ig2(self.X_0, adj, x, F.relu, self.adj_rho, A_orig=self.adj_orig)
        x = self.ig3(self.X_0, adj, x, F.relu, self.adj_rho, A_orig=self.adj_orig)

        # Pooling to obtain subgraph embeddings (try max pool for now)
        x = global_max_pool(x, subgraph_masks)
